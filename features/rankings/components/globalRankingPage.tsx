...
// 2026-09-13 재설계 — 상단(제목 · 모드 세그먼트 · 지표 탭과 지역)은 이전 그대로,
// 그 아래 내 순위 한 줄과 표. 정보가 적어 폭을 채우지 않고 640 가운데에 둔다.
export default function GlobalRankingPage({
    ...
    return (
        <PageContainer className="nl-global-rankings">
            <div className="nl-global-rankings__measure">
                ...
                {/* 한 페이지뿐이어도 ‹ 1 › 은 그리지만, 기록이 없으면 넘길 페이지도 없다 */}
                {data?.status === "available" && data.totalCount ? (
                    <Pagination
                        page={data.page}
                    ...
                    {data?.status === "available" && !busy && !result.isError
                        ? t("rankings.resultAnnouncement", {
                              start: data.totalCount
                                  ? (data.page - 1) * GLOBAL_RANKING_PAGE_SIZE +
                                    1
                                  : 0,
                              end: Math.min(
