...
## 최근 Grd 0 보완 (2026-09-20)

추가 DB 마이그레이션은 없다. 배포 후 최근 기록 연동을 다시 하면 기존 처리된 Grd 0 기록도 재검사한다. `ChartPlayHistory.grade_basic`은 수신 원본 그대로 두며 개인 최고 Grd와 새 연동 시점의 스냅샷·상위 50 합계·성장 이력을 갱신한다. 과거 Grd 이력은 소급해서 다시 쓰지 않는다. 플레이/FC/P 횟수는 재증가하지 않는다. 전체 연동 이전 기록은 전체 응답이 이미 덮는 범위이므로 재계산하지 않는다.

계산 조건: 양손(`is_onehand=false`), A/A2/S/P, 공식 상수 존재, 판정 5종 모두 유효한 정수이며 합계가 채보 노트 수와 일치, 최대 콤보가 노트 수 이내. 불확실한 입력과 검증되지 않은 B 이하·한손 계수는 추측하지 않는다. Real도 DB의 공식 상수·노트 수가 정확해야 하며, 누락된 수치를 표시 레벨로 대체하지 않는다. 정상 수신 Grd가 있으면 계산하지 않는다.

기존 최대 Grd 산식을 일반 판정으로 확장했다. 판정 가중치는 ◆JUST 1, JUST 0.3, GOOD 0.15, NEAR 0.05이고 랭크 계수는 A 7.2, A2 7.8, S 8.7, P 9.9다. 정수 연산으로 소수 둘째 자리 아래를 버린다. 구현 함수를 기존 공식 최근 기록 31건(A 1, A2 6, S 23, P 1)에 읽기 전용으로 대조해 31건 일치했다. 이 검증에서는 노트 수가 비어 있는 자료를 위해 판정 합계로 노트 수를 구성했으며, 실제 보완은 DB 노트 수까지 일치해야 수행한다. Real·한손·B 이하의 공식 일치 검증은 하지 않았다.

[공식 Op.3 안내](https://p.eagate.573.jp/game/nostalgia/op3/howto/entrance.html)는 Grd 시스템이 총 플레이 5회째부터 제공된다고 설명한다. 특정 계정의 Grd 0을 Basic Pass 미가입 때문이라고 단정하지 않는다. 원본 응답을 보관하지 않으므로 과거 응답의 직접 대조도 불가능하다.
