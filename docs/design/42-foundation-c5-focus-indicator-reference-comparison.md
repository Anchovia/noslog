...
| Field               | Value                                                                    |
| ------------------- | ------------------------------------------------------------------------ |
| Status              | `Research complete — visual comparison and user selection pending`       |
| Date                | `2026-08-09`                                                             |
...
focus token, component alias, or production implementation is approved. It does not
authorize a new guide specimen or select a candidate by itself.
...
|   4 | [Fluent 2 web alias color tokens](https://fluent2.microsoft.design/color-tokens2/) and [interaction color guidance](https://fluent2.microsoft.design/color)                                                                                                   | `colorStrokeFocus1`: Light white, Dark black; `colorStrokeFocus2`: Light black, Dark white. The standard web focus-outline helper uses the second color at `2px`; component recipes may vary.                                                                                           | Achromatic polarity maximizes contrast and avoids brand ownership.                                  | A normal Dark white outline conflicts with the approved restrained boundary direction and prior user feedback.                     |
...
Status: `Research recommendation for the visual comparison; not selected or approved`.
...
Status: `Not recommended under current authority`.
...
It avoids brand color but makes normal Dark focus white and normal Light focus black.
That treatment would dominate NosLog's quiet boundaries and contradict the approved
separation between ordinary structure and strong system overrides.
...
## Conditional `FI-A` Validation Contract

If the user selects `FI-A` after the same-condition visual comparison, its measured
validation specimen should hold these rules without yet promoting them to production:

1. Apply the normal authored indicator to keyboard-visible focus, not as a persistent
   pointer-click decoration.
2. Use the exact Spectrum Light/Dark color, `2px` thickness, and `2px` gap. Do not
   interpolate, lighten, darken, add glow, or substitute a Tailwind blue.
3. Do not recolor the component fill, text, icon, boundary, or selected state merely
   because it receives focus.
4. Leave `focus-inner` unassigned in the default recipe. Test whether an exact
   Spectrum static contrast alias is necessary only on imagery or filled contexts.
...
4. exact `2px` thickness and `2px` gap, radius/shape following, clipping, and visible
   perimeter area;
...
Any failure must reopen the source decision. The specimen may not silently tune the
Spectrum hue, create a new inner band, or borrow another system's geometry.
...
| `C5F-04` | Before selecting a source, build a same-condition visual comparison of the viable authored candidates, preserving each upstream system's exact color and geometry.          | `Required next gate`             |
| `C5F-05` | Leave `focus-inner` and static black/white exception mappings unassigned until a measured component context requires them.                                                  | `Proposed governance rule`       |
...
No candidate has been selected. The next task is a same-condition visual comparison
that lays out the viable authored approaches from this research without hybridizing
their values or geometry. The user selects the direction from that comparison; only
then does the selected candidate proceed to dedicated measured validation. Selection
would not approve production tokens, final component aliases, signature color,
feedback color, or application implementation.
