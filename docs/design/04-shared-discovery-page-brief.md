...
| `DISC-47` | **Filter vessel by width, closed.** Wide (1056+) keeps the visible rail; Intermediate (672–1055) and Compact use the result-covering filter layer with staging and `결과 보기` — the same "Compact contract" the Tier page already states for 768/1024. A trigger-anchored popover at Intermediate and/or Wide was drawn (Z1 ㉞) and rejected: it would give one page three vessels across widths, and the Bingo popover (`BINGO-25`) stays a bounded exception justified by its small filter set. Discovery has no Intermediate frames; this row fixes the rule for implementation. |
