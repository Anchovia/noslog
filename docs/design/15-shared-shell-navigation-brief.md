...
- **Approved refinement (2026-09-07):** Following the user's rejection of the
  near-full-width desktop result and selection of the osu! approach, apply
  Foundation 24's compact common-layout correction to all three wrappers.
  The global layout owns inline padding; individual page roots must not duplicate it.
- Header/footer backgrounds and horizontal boundaries continue across the viewport.
  ...
  to the centered header controls, not the viewport edge.
- This supersedes SHELL-34's viewport-edge content placement and the corresponding
  C8/P-page Figma geometry. Other C8 typography, color, anatomy, and behavior remain
  authoritative; Pretendard delivery follows the later user font decision.
- The rule also contains ordinary pages awaiting their individual implementation.
  Narrow reading measures remain available inside the common layout. The chart
  viewer/editor and `/admin/*` remain outside this change.
...
| SHELL-34 | Viewport-edge header/footer content placement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `Superseded — 2026-09-06 by SHELL-35` |
| SHELL-35 | Center header contents, ordinary page layout, and footer contents within the Foundation common maximum; use shared responsive inline margins and keep shell surfaces full width                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `Approved — 2026-09-06`               |
| SHELL-36 | **Stacked footer (below 840) centres its two rows** — `OrdinaryFooter · Compact` aligns the links row and the service notice to the centre, and the notice text is centre-aligned when it wraps; the single-row Wide footer (840+) keeps links left / notice right. The stacked footer is a colophon, not reading text: centring turns a wrapped notice's ragged right edge into a symmetric block and stops two small links from pretending to hold the content keyline. One variant change; 677 instances follow (Z1 ㉙)                                                                                                                                                                                                                                                                                                                                                                                    | `Approved — 2026-09-07`               |
| SHELL-37 | **Feedback / Error Report dialog drawn and sized: Compact 334 / Wide 768** (C8 `FeedbackDialog`, Z1 ㉚). The dialog keeps the approved modal flow (`SHELL-25` login round-trip and reopen) — a dedicated page would leave the screen being reported, and a header popover cannot carry attachment and error states. Body = `feedback.description`, TextArea `제보 내용` (`feedback.placeholder`, 10–1000자 counter), `이미지 첨부 (선택)`, actions `닫기` / `제출`; states ContentError (TextArea Invalid + `feedback.contentError`), AttachmentError (`feedback.invalidImage`), SubmitError (`feedback.submitError`), Submitting (Busy `제출 중`, other controls disabled), Success (`feedback.success` + `닫기`), LoginRequired (`feedback.loginRequired` + `로그인`). Vessel = ProfileShareDialog contract; the Wide width joins the media dialog at 768 so the overlay rule is two-step (see document 24) | `Approved — 2026-09-07`               |
