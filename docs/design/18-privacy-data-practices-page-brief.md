...
| PRIV-36 | High-fidelity 2026-09-02: the at-a-glance layer is four flat editorial groups (`emphasis-label` heading + body + underlined section link, divider-separated, no card chrome); 2×2 cards and a single summary card were rejected                                                                                                                                                                                                                                                                        | `Approved`                  |
| PRIV-37 | High-fidelity 2026-09-02: release blockers render as C4 `StatusMessage` Warning placed at the affected policy position (`법률 검토 대기` + explanation naming the temporary value); quiet inline parentheses were rejected as too easy to misread as final copy                                                                                                                                                                                                                                        | `Approved`                  |
| PRIV-38 | High-fidelity 2026-09-02: wide = prose at the approved `reading 768` container on the left with a sticky 292 contents column on the right (GitHub privacy statement measured 720 prose + 384 right TOC; MS Learn same grammar); compact keeps the top `이 페이지에서` disclosure per this brief. Korean copy set approved (at-a-glance group texts, `최종 수정`/`시행` labels, reorganized section labels, age rule, no-collection bullets, device-storage rows, version-history labels, blocker text) | `Approved`                  |
