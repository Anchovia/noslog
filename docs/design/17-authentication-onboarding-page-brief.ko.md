...
- 초기 계정 언어는 명시적인 승인된 비로그인 설정이 있으면 이를 상속하고,
  없으면 승인된 브라우저 Locale 기본값을 따릅니다.
...
- 비로그인 기기 설정이 소유하는 Theme Control;
- 이 한 화면 작업의 진행 표시기.
...
| AUTH-14 | 이전 방향은 신규 계정에 명시적 비로그인 언어·번역 제목 설정을 상속했습니다.                  | `대체됨` |
...
| AUTH-28 | 신규 계정에는 명시적 비로그인 언어 설정만 상속하고 번역 제목 Disclosure는 설정으로 두지 않음 | `승인`   |
