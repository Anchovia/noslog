...
- **악곡 상세(2026-09-16 재설계):** **머리** = 자켓 80(위 고정, 카테고리 배지 없음) · 카테고리 배지(제목 바로 위 왼쪽, 배지 → 제목 4) · 제목(페이지 제목 고정 · 글자 줄이기 없음 · 한 줄, 넘치면 끝 32 페이드) + 제목 글자 끝 8 뒤 내 등급 공식 아이콘 = 제목 줄 높이(폰 32 · 1056 이상 40, 제목 줄 세로 가운데 · 기록 있을 때만) → 번역 제목(설정에서 켰을 때, 본문 보조 · 기본 글자색 · 제목에 붙임 · 한 줄 · 등급 자리 앞에서 끝 페이드) →8→ 아티스트(흐린 본문 보조 · 전체 폭 한 줄 페이드) — 잘린 글자가 있으면 글자 묶음을 눌러 모두 펼침(펼치면 단어 단위 줄바꿈 · 등급은 첫 줄 높이) · 글자 묶음이 80보다 짧으면 자켓 옆 세로 가운데 →16→ 난이도 세그먼트
  →12→ 수치 띠(선택한 난이도의 공식 레벨(라벨 「공식 레벨」, 칸 값 = 서열 값 그라데이션 색) + 공개 서열 값 Basic S · 990k · Pianist · Recital, 미등재는 칸 없음) →12→ 액션(있는 것만 M 보조,
  없으면 줄 없음) →16→ **1단 탭 4개 「개요 · 내 기록 · 랭킹 · 평가」**(주소 `tab` 은 `detail · record · ranking · tier` 그대로).
