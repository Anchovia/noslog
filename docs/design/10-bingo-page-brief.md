...
| BINGO-21 | High-fidelity 2026-09-02: list cards inherit the current product's card anatomy (user-supplied screenshot as the primary reference) — full-bleed square cover, mini-board overlay chip top-right (`surface/media-scrim` + white cells, optical gap 2, signed-in only), title, divider, then `칸 n/25` left and total-nos right pinned to the card bottom; rows in a grid equalize height by a measure-then-fix-then-fill pass                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `Approved` |
| BINGO-22 | The cover's bottom edge doubles as the progress bar (user idea; the YouTube watched-progress grammar): track = white 30%, fill = `local-data/single`, ratio = cells/25, full-bleed, signed-in only — a white fill was rejected because it perceptually inverts on light art                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `Approved` |
| BINGO-23 | Board-cell completion is the background-fill grammar observed on Bingosync and Bingo Blitz: a completed cell inverts its background to ink with the coordinate staying in place in `primary/on-primary`; selection = 2px `border/strong`; chance = dashed 1px; check glyphs, art-reveal, and dauber-stamp treatments were all rejected. Coordinates are `A1–E5` as in the current product                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `Approved` |
| BINGO-24 | Detail keeps the current product's bones (identity band → progress/reward → 5×5 → filter chips with counts → mission rows with coordinate, instruction, category, and a C4 checkbox) with four repairs: square 68 cells (board height halves), the duplicated line counter removed from the identity band, the reward structure exposed as `필요 줄 n · 줄당 x nos · 풀보드 +y nos` beside `총 z nos`, and board↔mission selection sync visualized via `interaction/menu-set`. Korean copy for the new strings approved 2026-09-02 (`최근 기록한 빙고`, `해금 완료`/`풀보드 완료`, sorts, reward labels, reset dialog set; third batch 2026-09-02: list loading `빙고를 불러오는 중입니다.`, batch-busy `불러오는 중`, reset failure `빙고 기록을 초기화하지 못했습니다.` + `기록은 그대로 남아 있습니다. 다시 시도해 주세요.`). **The catalog-summary sentence was removed 2026-09-02** (the copy-budget rule: the manual-record meaning is already carried by the record-verb labels, the login prompt, and the catalog itself — the list identity is the `빙고` heading alone); ja/en drafts approved 2026-09-02 except the removed line | `Approved` |
...
### Decision Log addendum — 2026-09-07 · `BINGO-25` (Z1 ㉘)

| ID       | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BINGO-25 | **List filter/sort at Intermediate and above is a popover under the trigger, not a result-covering layer.** C8 `BingoFilterPopover`: 334 wide, DestinationPanel vessel (`surface/overlay`, `border/overlay` 1 px, `radius/overlay`, overlay elevation, inset 8, gap 4), two radio groups — `상태` (전체 · 진행 중 · 해금 완료 · 풀보드 완료 · 빙고 찬스) and `정렬` (수록순 · 최근 기록순 · 진행률 높은 순) — with `metadata` group labels and a `border/divider` rule; the list's existing filter and sort set (`bingo.catalog.*`), **meaning unchanged**. (Correction 2026-09-07: the first specimen had used the detail page's mission-filter keys `bingo.filter.*` / `bingo.sort.*` — 완료 / 진행순 / 진행률 낮은 순 — by mistake; the vessel decision stands, the content is the list's own.) Choices apply immediately (no staging, no commit button); while results refresh the list takes `content/pending` (`DISC-44`) and the trigger shows Applied. Compact keeps the result-covering layer with `결과 보기` (`DISC-09`). Rationale: the file already splits shells this way (More panel Compact modal / Wide popover, exam selector 1B popover), the bingo filter set is small enough for one popover, and on a wide screen the results stay visible while choosing. |
