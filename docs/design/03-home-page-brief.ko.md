...
| 일반 NosLog 공지        | 각 일반 업데이트를 주 목적지와 데이터 연동 뒤의 하단 업데이트 영역에 한 번만 표시합니다.                          | `승인` |
...
레퍼런스만으로 정확한 목적지 순서, 카드 비율 또는 NosLog 공지 발행 모델을
정할 수 없습니다. 해당 내용은 사용자 결정으로 남깁니다. 공식 소식 표현은 아래와
같이 승인됐습니다.
...
일반 업데이트의 정확한 수량과 표현은 미확정입니다. 검색 위에 같은 긴급도를
가진 Banner 여러 개를 쌓으면 안 됩니다.
...
| 여러 일반 공지              | 하단 NosLog 업데이트 영역에 한 번씩 표시하고 검색 위의 같은 긴급도 Banner 묶음으로 만들지 않습니다.                    | `승인` |
...
| HOME-15 | 일반 공지 표시 수량     | 하단에 표시할 일반 NosLog 업데이트 수와 과거 공지 접근 방식을 결정                                                            | `미확정` |

## 첫 번째 논의 묶음

다음 홈 기획서 개정 전에 사용자가 아래 내용을 결정해야 합니다.

1. `HOME-15`의 일반 공지 표시 수량과 과거 공지 접근 방식을 결정합니다.

이 콘텐츠 결정을 해결한 다음 묶음에서 데스크톱 구성, 정확한 Copy 및 상세
검색 상태를 논의합니다.
