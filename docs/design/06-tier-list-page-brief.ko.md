...
- 시각적인 간단 카드에는 제목, 난이도, Grd 또는 레이팅 기여도를 반복하지
  않는다.
...
- jacket 아래에는 다음 순서로 표시한다.
    1. 원문 악곡 제목;
    2. 선택 채보 난이도와 공식 레벨;
    3. 존재하는 경우 해당 채보의 선택 모드 공식 Grd 기여도; 그리고
    4. 적용 가능한 `Basic · Pianist` 맥락에서만 NosLog 레이팅 기여도.
...
- 간단히 보기와 자세히 보기 모두 원문 악곡 제목을 유지합니다. 승인된 번역·읽기는
  검색 가능하게 유지하고 악곡 상세에서만 Disclosure로 제공합니다.
...
- 긴 일본어 원문 제목 및 한국어·영어 번역 제목이나 일본어 읽기 별칭으로 찾았을 때도
  원문만 표시되는 악곡;
