...
- 일본어 읽기 제목은 플레이 행에 한 줄을 추가하므로 한 언어를 전제로 한 고정
  행 높이가 안전하지 않음을 보여준다.
...
- 플레이 항목은 세로로 훑기 쉬워야 한다. 자켓 비율은 `1:1`을 유지한다. 번역·읽기와
  원문 제목은 Metadata 위에 겹치지 않고 높이를 늘릴 수 있다.
...
- 프로필 플레이 항목에서 사용자의 번역 제목 표시 설정을 따른다.
- 켜져 있으면 승인된 탐색 계약의 제목·아티스트 계층을 보존하면서 번역 제목 또는
  일본어 읽기를 더 큰 원문 악곡 제목 위의 가장 작은 보조 행에 둔다.
- 유효한 한국어, 일본어, Latin, 혼합 Script 사용자명은 전체 접근 가능한 이름을
  유지할 때만 Wrap 또는 시각적 Truncate할 수 있다.
- 일본어 읽기와 긴 영어 번역은 한 줄을 추가할 수 있다. 고정 한 줄 항목 높이에
  의존하지 않는다.
...
12. 한국어 번역, 일본어 읽기, 긴 일본어 원문, 긴 영어 번역과 자켓 없음 악곡;
...
- Wide Layout은 무관한 Dashboard가 되거나 제외 필드를 노출하지 않고 추가 읽기·
  비교 공간을 사용한다.
- 한국어, 일본어 읽기, 원문 제목과 긴 영어 콘텐츠가 의미 손실 Truncate나 고정 높이
  충돌 없이 Reflow한다.
