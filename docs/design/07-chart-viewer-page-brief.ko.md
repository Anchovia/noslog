...
- Server는 `publishedRevision`이 null이 아니며 유효한 `publishedContent` 문서인 경우만
  ...
  된다.
- 공개 Payload는 원문 제목, 사용자의 표시 설정에 따른 승인 번역 제목 또는 일본어
  읽기, 아티스트, 난이도, 공식 레벨, 재킷, 노트 수, 재생 시간 및 공개 Revision을
  포함한다.
...
- 뒤로가기
- 원문 악곡 제목
- 사용자 설정이 켜져 있을 때 승인된 번역 제목 또는 일본어 읽기
- 아티스트
- 난이도와 공식 레벨
- 노트 수와 채보 재생 시간
...
- Label, 오류 메시지, 설정 또는 제목 Metadata에 고정 너비를 가정하지 않고 한국어,
  일본어 및 영어를 지원한다.
- 원문 악곡 제목을 보존한다. 활성화된 번역 제목 또는 일본어 읽기를 원문을 교체하지
  않는 보조 정체성으로 보여준다.
...
- 직접 공유 URL, 악곡 상세 진입, Browser Back, 명시적 뒤로가기 및 기본값이 아닌 악곡
  상세 콘텐츠 영역으로 복귀
- 원문 일본어 장문 제목, 긴 한국어 번역, 긴 영어 번역, 일본어 읽기, 번역 없음 및
  아티스트 없음
