...
소유권, 언어·테마 동작, 프로필 이미지·NosLog 닉네임·NOSTALGIA
...
- **승인:** 비로그인 사용자는 사용할 수 없는 계정 Form을 마주치지 않고 언어와
  테마를 바꿀 수 있으면 성공합니다.
...
- 인증 Session을 지우지만 기기 단위 테마와 저장된 비로그인 브라우저 언어 설정은
  보존합니다.
...
| 사용자 설정 모델                        | System Theme Migration, 계정·브라우저 언어 우선순위, 5개 긍정형 공개 개념                                |
...
5. System·Dark·Light 적용, 기존 선택 Migration, 운영체제 변경, Reload 유지 및
   두 시각 테마;
6. 기존 번역 제목 설정을 제거하되 번역·읽기 검색 별칭과 악곡 상세 Disclosure는
   잃지 않는지;
...
- **로그인 전용 설정 — 거절:** 비로그인 사용자도 인증 없이 언어와 테마 설정을
  실제로 사용할 필요가 있습니다.
...
- **계정에 테마 동기화 — 거절:** 기기 환경이 더 강한 소유자입니다. 언어는
  계정 인식 설정으로 유지합니다.
...
| SET-10 | 이전 방향은 즉시 적용되는 번역·읽기 제목 표시 설정을 추가했습니다.                              | `대체됨` |
...
| SET-38 | 번역·읽기 제목 설정을 제거하고 원문 제목을 상시 유지하며 번역 Disclosure는 악곡 상세에서만 제공 | `승인`   |
