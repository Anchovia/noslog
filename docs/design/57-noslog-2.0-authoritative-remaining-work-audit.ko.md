...
`1`–`3`은 계속 완료 상태입니다. 다음이자 유일한 남은 실행 단위는 `4`–`6`이고
`4 · Data visualization`부터 시작합니다. Motion 조사, 후보 비교, 시편, 사용자 결정,
검증 및 한영 통합을 별도 작업 단위로 늘리지 않습니다. 잠긴 viewer/editor는 계속
제외합니다.
