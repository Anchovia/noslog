...
- 작성 시작일: 2026-07-29
- 승인일: 2026-07-30
- 마지막 결정 갱신일: 2026-08-08 (`IA-21` / 홈 `HOME-18`)
- 문서 언어: 한국어 동기화본
...
| [Google Search settings](https://www.google.com/preferences)                                                          | 공개 설정 Surface는 로그인 전에도 언어·테마·브라우저 단위 이용 환경 설정을 제공할 수 있습니다.                                         | NosLog의 언어와 테마는 계정 전용 컨트롤이 아니라 유효한 비로그인 설정입니다.                                                                     | Google 검색은 더 넓은 설정과 다른 개인화 모델을 가집니다.                          |
...
- 피드백과 공식 소식은 독립된 주 홈 블록으로 격상하면 핵심 첫 진입 과업과
  경쟁합니다. 데이터 연동은 이후 승인된 홈 계약에 따라 필수 동등 목적지와
  더보기 Panel 진입으로 유지합니다.
...
| 신원, 설정 및 신뢰 | “서비스에 어떻게 진입하고 설정하며 신뢰할 수 있는가?”          | 로그인, 온보딩, 공개 설정, 로그인 프로필·계정 설정, 언어, 테마, 개인정보, 탈퇴, 점검, 오류 및 찾을 수 없음 상태 | 유틸리티 및 Lifecycle 패밀리                        |
...
| 공식 채보 기여     | “공식 NosLog 채보 개선에 어떻게 기여할 수 있는가?”             | 개인 채보 Draft, 로컬 음원 편집, Preview, Revision 이력, 불변 심사 제출본, 수정 요청, 반려, 승인된 기여자 표기  | 로그인 기여 흐름이며 공개 대체 채보 Catalog가 아님  |
...
- 사용자용 에디터는 공식 NosLog 채보 기여를 위해 기존 채보 작업 공간을
  재사용합니다. 작성자는 자신의 Draft와 심사 제출본만 소유하며 공개 Canonical
  채보 교체는 관리자만 할 수 있습니다. 공개 커뮤니티 채보 Catalog는 만들지 않습니다.
...
flowchart TD
    ...
    AUTHOR["공식 채보 기여<br/>개인 Draft · 심사 제출"]
...
- 제품 목적지 다음의 시각적으로 분리된 유틸리티 영역에 설정과 피드백·오류
  제보를 제공합니다. 보이는 유틸리티 그룹 라벨 없이 시각적 Divider를
  사용합니다.
...
- 관리자 UI 재설계는 NosLog 2.0 사용자 범위 밖이지만 권한 있는 관리자 진입은
  별도 블록으로 유지할 수 있습니다.
- 정확한 2열 목적지 순서, Compact Modal 동작, Wide Popover 동작, Scroll
  가시성 및 셸 변형은
  [15-shared-shell-navigation-brief.ko.md](./15-shared-shell-navigation-brief.ko.md)에서
  승인했습니다.
...
- 비로그인·로그인 사용자가 함께 사용하는 하나의 Locale Prefix 설정 목적지
  ...
  않습니다.
- 서비스 표시 언어와 테마를 항상 제공합니다. 이 설정은 공개 서비스 이용 경험에
  직접 영향을 줍니다.
...
- 테마는 기기 단위로 유지합니다. 언어는 기존 계정으로 로그인하면 계정 설정을
  우선하고, 새 계정은 명시적인 비로그인 선택으로 초기화합니다. 로그아웃하면
  저장된 브라우저 단위 비로그인 설정으로 복귀합니다.
...
- 승인된 홈 기획서는 악곡 → 채보 뷰어 → 서열 → 랭킹 → 빙고 → 검정 → 오락실
  → 데이터 연동 순서로 같은 크기의 동등 목적지 여덟 개를 확정합니다.
- 데이터 연동을 여덟 번째 홈 블록과 안정적인 더보기 Panel 진입으로
  유지합니다. 이전의 별도 홈 행은 대체됐습니다.
- 지원하는 모든 너비에서 정확히 4열 2행을 사용합니다. 블록 치수, Gap,
  Padding 및 라벨 줄바꿈은 사용 가능한 공간에 맞게 조정할 수 있지만 같은
  목적지, 의미적 순서 및 `4 × 2` 관계를 유지합니다.
...
3. **이동 블록:** 악곡, 채보 뷰어, 랭킹, 서열, 빙고, 검정, 오락실 및 데이터
   연동의 직접 블록을 승인된 `4 × 2` Collection에서 서로 분리해 유지합니다.
   통합 콘텐츠 라벨을 도입하지 않습니다.
4. **편집 콘텐츠:** 핵심 제품 과업 다음에 최신 일반 NosLog 공지 Link 세 건을
...
- **데이터 연동:** 제거하지 않습니다. 홈 목적지 Collection의 여덟 번째 동등
  블록과 더보기 Panel에 안정적인 경로를 유지합니다.
...
| ----------------------- | ---------------------------------- | ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| 악곡 빠르게 찾기        | 홈 검색, 악곡 블록 또는 더보기     | 악곡 범위 열기 → 검색·필터 → 악곡 결과 → 악곡 상세                 | 사용자가 번역·읽기로도 검색해 올바른 악곡 정보에 도달하고 상세에서 필요한 경우 이를 확인합니다. | 결과 없음, 모호한 제목, 없는 난이도                                  |
...
| 빙고 진행               | 홈 블록 또는 더보기 → 빙고         | 빙고 선택 → 5×5 미션 확인 → 진행 갱신 → 해금·NOS 이해              | 사용자가 악곡 해금 또는 NOS 보상까지의 진행 상태를 이해합니다.                                  | 로그인 필요, 유효하지 않은 빙고·미션, 저장 실패                      |
...
| 인증 및 계정 완료       | 헤더 Login 또는 계정 전용 행동     | 문맥형 Login → Discord OAuth → 신규면 최소 온보딩 → 안전 복귀      | 사용자가 완전한 NosLog 신원으로 원래의 검증된 목적지에 도달합니다.                              | OAuth 취소·만료, 제공자 실패, 닉네임 중복, 로그아웃 후 공개 둘러보기 |
...
- 데이터 연동은 홈 목적지 Collection과 더보기 Panel에서 접근할 수 있어야
  합니다.
- 설정은 로그인 여부와 관계없이 더보기 Panel의 하나의 항목에서 접근할 수
  있어야 합니다. 로그인 여부는 제공되는 설정을 바꾸지만 목적지 정체성을
  바꾸지 않습니다.
- Login은 Discord OAuth와 필요한 온보딩 전체에서 검증된 내부 목적지 하나를
  보존해야 합니다. 미완료 계정은 간결한 목적지 이유와 함께 온보딩으로
  Redirect되고 명시적으로 로그아웃해 공개 홈으로 나갈 수 있습니다.
...
- 고정 하단 내비게이션을 추가하지 않습니다.
- Compact 레이아웃에서는 아래로 스크롤할 때 헤더를 숨기고 위로 스크롤할 때
  다시 표시합니다. 넓은 Desktop 레이아웃에서는 계속 보이는 Sticky 상태를
  유지합니다.
- Panel은 모든 너비에서 승인된 같은 2열 의미 순서를 사용합니다. Compact에서는
  헤더 아래 Full-width Modal, Wide에서는 오른쪽 Anchored Non-modal Popover입니다.
...
- **승인:** 경로가 합의한 계층을 막거나 명확한 사용성 문제를 만들지 않는 한
  첫 디자인 마일스톤에서는 현재 사용자 경로 Slug를 유지합니다.
- **승인:** `/tiers/[slug]`를 노출된 IA Node가 아니라 호환성 동작으로
  취급합니다.
...
| IA-05 | 데이터 연동 위치           | `IA-21`과 홈 `HOME-18`로 대체; 이전 결정은 안정적인 더보기 진입을 유지하면서 별도 홈 행을 사용                                                | `대체` |
...
| IA-11 | 더보기 Panel 콘텐츠와 순서 | 승인된 2열 순서로 제품 목적지 8개, Divider, 설정·피드백 유틸리티, 조건부 관리자를 배치하고 그룹 제목은 사용하지 않음                          | `승인` |
| IA-12 | 헤더 스크롤 동작           | Compact에서는 아래로 숨고 위로 표시하며 넓은 Desktop에서는 계속 보이는 Sticky 상태 유지                                                       | `승인` |
...
| IA-19 | 인증 및 온보딩             | Discord 행동 하나, 공개 둘러보기 대안, 두 필드 완료, 미완료 프로필 Gate, 로그아웃 이탈 및 검증된 목적지 복귀                                  | `승인` |
| IA-20 | 공식 채보 기여             | 로그인 개인 Draft와 공식 채보 심사용 불변 제출본, 관리자 전용 Canonical 공개, 대체 채보 Catalog 없음                                          | `승인` |
| IA-21 | 홈 목적지 Collection       | 데이터 연동을 포함한 같은 크기의 동등 블록 여덟 개를 지원하는 모든 너비에서 정확히 4열 2행으로 사용하고 데이터 연동의 별도 더보기 진입은 유지 | `승인` |

## 단계 승인

사용자는 동기화된 영어·한국어 문서와 해결된 결정 기록을 검토한 뒤
2026-07-30에 이 정보 구조 단계 전체를 승인했습니다. 당시 시각 구성, 정확한
반응형 순서 및 페이지 고유 상태는 페이지 기획서 결정으로 남아 있었으며, 해당
승인이 그 내용을 암묵적으로 확정하지는 않았습니다. 이후 사용자가 승인한 홈
기획서에서 페이지 패밀리 계층은 변경하지 않고 `IA-07`과 `IA-18`의 편집
목적지를 구체화했습니다. 2026-08-08에는 홈 `HOME-18`과 `IA-21`이 `IA-05`의
별도 데이터 연동 홈 행을 대체하고 지원하는 모든 너비에서 여덟 동등 목적지
Collection을 `4 × 2`로 확정했습니다. 이후 승인된 공용 탐색 기획서는 이 계층을 변경하지
않고 비로그인 개인 컨트롤 동작과 악곡·채보 범위 계약을 구체화했습니다. 이후
승인된 공통 셸 기획서는 같은 정보 구조를 보존하면서 전역 목적지 순서, Compact
Modal, Wide Popover, Header 가시성, 셸 변형 및 Footer 소유권을 확정했습니다.
이후 승인된 인증 및 온보딩 기획서는 공개 페이지 계층을 바꾸지 않고 Discord
진입, 공개 둘러보기 대안, 최소 완료 Gate, 안전한 복귀 및 복구를 확정했습니다.
이후 승인된 개인정보 및 데이터 처리 기획서는 계층형 공개 방침 구조, 만 14세
...
이후 승인된 채보 기여 에디터 기획서는 관리자 셸을 공개하거나 별도 탐색 Catalog를
만들지 않고 악곡 아래에 로그인 사용자용 하위 흐름을 추가했습니다.
...
- 승인된 결정, 관찰 근거 및 해결되지 않은 출시 차단 조건을 눈에 띄게 구분합니다.
- 주요 흐름에 성공 및 복구 Branch를 포함합니다.
- 내비게이션 컴포넌트가 달라도 모바일과 데스크톱은 같은 의미적 계층을
  사용합니다.
- 라벨을 확정하기 전에 한국어, 일본어 및 영어 제약을 포함합니다.
- 페이지 기획서 작업을 시작하기 전에 사용자가 해결된 결정 기록을
  승인했습니다.

## 다음 작업

1. 이후 페이지 기획서, Foundation Specimen, 후속 디자인 및 구현 연결에 승인된
   공통 셸 계약을 일관되게 적용합니다.
2. 전역 셸을 확장하지 않고 페이지 고유 콘텐츠, 행동 및 상태는 해당 기획서에
   유지합니다.
3. 하위 목적지와 복귀 상태를 정의할 때 승인된 홈·공용 탐색 계약을 보존합니다.
4. 이후 명시적으로 승인된 결정이 기록된 항목을 대체하지 않는 한 이번
   승인된 정보 구조를 유지합니다.
5. 목적지 페이지 안에 제공자·완료 Logic을 중복하지 않고 모든 계정 전용 행동에
   승인된 인증 및 온보딩 계약을 적용합니다.
