...
- 관리자 UI 재설계는 NosLog 2.0 사용자 범위 밖이지만 권한 있는 관리자 진입은
  별도 블록으로 유지할 수 있습니다.
- 정확한 목적지 순서, 유틸리티 링크 포함 여부, 열 수, Panel Anchoring,
  Overlay 동작 및 열기·닫기 Motion은 페이지 셸 디자인 결정으로 남깁니다.
...
5. **편집 및 지원 콘텐츠:** 공식 소식과 피드백은 핵심 제품 과업 다음에
   유지합니다.
...
| IA-11 | 더보기 Panel 콘텐츠와 순서 | 악곡, 채보 뷰어, 랭킹, 서열, 빙고, 검정, 오락실, 데이터 연동을 독립 블록으로 포함하며 정확한 순서와 유틸리티 링크는 미확정 | `미확정` |
...
1. 승인된 목적지 블록 외에 더보기 Panel에 반드시 필요한 유틸리티·신뢰 링크는
   무엇이며 어떤 의미적 순서로 배치해야 할까요?
2. 피드백은 주요 과업 계층 밖의 안정적인 지원 목적지로 유지할까요?
3. 공식 소식은 핵심 홈 과업 다음의 편집 콘텐츠로 유지할까요?
...
1. 사용자와 IA-06, IA-07 및 IA-11의 남은 순서·유틸리티 부분을 검토하고
   해결합니다.
