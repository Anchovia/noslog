...
| ANN-30 | Archive rows reuse the Home announcement-row anatomy (title `body`, date `metadata` subdued, divider rhythm, title as the only link); the back control follows `SET-42`; wide layouts use a centered `reading 768` column (corrected 2026-09-01 — the left-aligned P7 measure is minimal-shell-only) | `Approved` |
