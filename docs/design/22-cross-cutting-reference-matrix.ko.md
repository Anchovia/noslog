...
| `PR-08` | **절제된 계층, 콘텐츠 중심 정체성**                | 주 행동과 고빈도 행동은 즉시 사용할 수 있게 유지하고 보조·맥락적 행동만 점진적으로 공개한다. 장식적인 상시 컨트롤 대신 자켓 이미지, 타이포그래피, 비례, 공간 리듬, 표면, 아이콘, 문체와 목적 있는 모션으로 NosLog 정체성을 만든다.                                                                                                                                                               | `Approved` |
...
| `PR-08` | 주 행동과 고빈도 행동은 보이고 사용할 수 있게 유지한다. scent와 복귀가 명확할 때만 보조·맥락적 행동을 낮추거나 그룹화하거나 disclosure한다. 내비게이션에는 링크를 유지하고 도메인이 요구하는 명시적 동등 선택을 보존한다. 비교적 차분한 과업 셸을 사용해 자켓과 NOSTALGIA 콘텐츠가 표현을 주도하게 한다. 고밀도 랭킹·폼·설정·에디터 컨트롤은 더 차분하게 유지하고 홈·악곡 정체성·제한된 편집 순간은 더 표현적일 수 있다. 뷰어 chrome은 WebGL 채보보다 두드러지면 안 된다. 모션에는 목적과 reduced-motion 처리가 필요하다. | 시그니처 hue, 서체, type scale, artwork 처리, 표면 방식, 아이콘 스타일, 버튼 수, 메뉴 구조, 모션 duration 또는 페이지 패밀리 표현 강도 값. 화면마다 주 버튼 하나를 기계적으로 강제하거나, 승인된 고빈도 컨트롤을 숨기거나, 버튼을 장식으로 쓰거나, 여러 표면에 행동을 중복하거나, 모든 카드를 raised·accented 처리하거나, NosLog를 개성 없는 중립 유틸리티 UI로 축소하지 않는다. |
...
| `REF-D02` | 이 매트릭스를 교차 영역 출처 풀로 취급하며 집중된 12~15개 출처 결정 조사를 대신하지 않는다.                              | 승인된 조사 방법은 집중된 각 주요 결정마다 관련 근거가 포화될 것을 요구한다.                                                                                                                                                                                                                                                                                                                                                     | `Proposed` |
...
| `REF-D05` | foundation v0.1을 작성하기 전에 10개 원칙 후보를 사용자 검토로 가져간다.                                                 | 디자인 가이드 워크플로와 승인 게이트가 요구한다.                                                                                                                                                                                                                                                                                                                                                                                 | `Proposed` |
