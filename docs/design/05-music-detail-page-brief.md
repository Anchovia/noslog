...
- Last decision update: 2026-08-13
- Language: English
...
| Ranking           | Sign in to see my rank / Participants / S-or-higher distribution          | `로그인하고 내 순위 보기` / `참가자 N명` / `S 이상 점수 분포` |
...
3. explicit pagination when required; and
4. the secondary high-skill score distribution.
...
- The row is one line with fixed columns in this order: rank, profile image, player
  name, score grade, score, achievement mark. Only the name column flexes. Every other
  column keeps its width even when its value is absent, so a missing grade or
  achievement never shifts the score column.
...
- Only rank, player, and score carry a visible column label. The profile, grade, and
  achievement columns are labeled for assistive technology only, because their content
...
- Full Combo and Pianist appear as short labelled marks, `FC` and `P`, using the
  approved achievement colour role. Colour never carries the achievement alone.
...
- The conditional current-user summary above the list repeats this column geometry and
  the same grade and achievement treatment, so the two read as one system.
...
| MDET-27 | Ranking hierarchy                               | On mobile order conditional current-user context, leaderboard, pagination, then secondary score distribution; the visible participant heading and visible result-range line were removed as repeated context on 2026-08-13, keeping a programmatic region name and the assistive-technology range announcement                                                                                          | `Approved — 2026-08-13`        |
...
| MDET-84 | Leaderboard row composition                     | Use one line with fixed columns (rank, profile, name, grade, score, achievement) where only the name flexes and absent values keep their column; size the score column to the widest score; label only rank, player, and score visibly; deliver grade as a self-hosted official image and Full Combo or Pianist as labelled `FC` and `P` marks; follow the name's own writing system for its text style | `Approved — 2026-08-14`        |
