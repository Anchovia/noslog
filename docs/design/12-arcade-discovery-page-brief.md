...
- ~~Compact layout is list-first and provides one clear `Map view` action.~~ **Amended
  2026-09-02 (`ARCADE-32`):** compact discovery opens with a fixed-height synchronized
  map preview above the list; tapping the preview opens the focused full map view. Do
  not add a bottom navigation or permanent competing List/Map tab bar solely for this
  page.
...
| ARCADE-04 | Make mobile list-first with one map action and wide layouts a list/map split                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `Superseded` |
...
| ARCADE-28 | Result rows are independent cards (C6 surface/radius-8 contract): name `entity-title` + region, availability summary as text, preference/distance metadata                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `Approved`   |
| ARCADE-29 | Map symbols: dark rounded square + `{n}곳` = cluster (arcade count); `local-data/single` area-proportional bubble + white count = preferred users (fixed national scale, 20/28/38 caps); small subdued dot = below-threshold venues; selection = 2px ring; legend always available. The map canvas itself is the runtime Kakao render — Figma draws a representative placeholder excluded from token audits                                                                                                                                                                                                                                                                                                                                                                                                    | `Approved`   |
| ARCADE-30 | Approve the Korean copy set: `내 주변` · `이 지역에서 검색` · `지도 보기`/`목록 보기` · `가동 기체 있음` · sorts `기본 순 (지역·이름)`/`이름순`/`선호 인원순`/`거리순` · `{n}대 중 {m}대 가동`/`주의 기체 포함`/`기체 가동 미확인` · `가동` · `기체 {n}` · `기체 정보 {n}일 전 확인`/`영업시간 최근 미확인` · `선호 집계 준비 중` · location-failure lines · `길찾기` · `정보 제보` + seven report types · four legend lines · unit `{n}곳`; ja/en drafts for the full set approved 2026-09-02 (native review still pending)                                                                                                                                                                                                                                                                                   | `Approved`   |
| ARCADE-31 | Map-at-top persistent discovery was re-examined on 2026-09-02 against 25+ locator references and the rhythm-game domain (KONAMI, SEGA, and Musecat locators are all list-centric; the SEGA locator has no map at all) and stays rejected; instead the single `Map view` action floats fixed at the compact viewport bottom (Airbnb/Google convention) so it is discoverable without scrolling                                                                                                                                                                                                                                                                                                                                                                                                                  | `Superseded` |
| ARCADE-32 | Compact discovery opens with a fixed-height interactive map preview above the list (user decision 2026-09-02: a picture orients faster than text): the preview shows the same shared result set with the approved ARCADE-29 symbols, tapping it opens the full map view, and the list follows immediately below; this supersedes ARCADE-04's list-first-with-one-action composition and the ARCADE-31 floating button, while the map-first full-screen composition and persistent List/Map tab bar remain rejected                                                                                                                                                                                                                                                                                             | `Approved`   |     |
| ARCADE-33 | Second copy pass approved 2026-09-02: `오락실을 불러오는 중입니다.` · `기체 정보 최근 미확인` · `선호 오락실을 지정하지 못했습니다.` · `기체 미지정` · busy label `지정 중`; report-layer copy otherwise reuses the `feedback.*` catalog                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `Approved`   |
| ARCADE-34 | Cross-width layout system approved 2026-09-02 after a full reset: Detail uses one card system at every width — compact 1B (photo hero, then location/hours card with inline copy-icon address, cabinet card carrying the availability summary and scoped verification in its head, price/contact/notes card) and wide 2A (title, Airbnb-style photo grid, body cards 805 plus a sticky action rail 395); Discovery keeps 3A (preview 180) and 4A (list 395 / map 805); 768 is the one-column rendition of the same cards, never a separate structure. Photos are the default fixture (the photoless venue is a state variant), identity-band region/summary/verification lines are removed as duplicates, and address copy is an inline `Icon/copy` control (44/40 target, ink gap 4) instead of a text button | `Approved`   |
...
| `ARC-DV-01` (amended 2026-09-04, same day) | **Cluster bubble = `surface/media-scrim` (60 %) face + `content/on-media` label.** `primary/default` was adopted first, but the Kakao map stays light in Dark while `primary/default` flips to #DBDBDB — bubble-vs-map 1.20:1. The over-media pair holds 5.5:1 (bubble vs map) and 6.33:1 (label) in both modes. Legend row swatch follows; legend copy `파란 원` → `숫자 원`. The floating `이 지역에서 검색` button over the map is likewise styled as an overlay over media (`surface/overlay` + `border/overlay` + overlay shadow). |

### Decision Log addendum — 2026-09-06 (document 86 item 5.7 · D-7)

| ID          | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ARC-DV-02` | **Every discovery result row carries a 64 px venue-photo thumbnail** on the left (Z1 ㉒ D-7 (b)). Scope resolved by the user: all result rows in the list and in the Wide list column — not only a selected-venue preview. Row = horizontal, gap 12, thumbnail 64×64 `radius/container`, content column FILL with the existing head / availability / meta stack (gap 4); padding 12/16 unchanged, so the row stays 92 (88 where the meta line is absent). The photo comes from the venue's limited photo policy; a venue without a photo shows the empty-slot convention (`surface/sunken` + `border/empty-slot` + `Icon/map-pin`) rather than dropping the slot, so rows keep one keyline. Detail-page cards are not rows and are unchanged. |
