...
| Share preview and actions              | [`features/profile/components/profileShareDialog.tsx`](../../features/profile/components/profileShareDialog.tsx)                                                                                                                                                                                                 | Add selected-mode preview, capability detection, save/copy fallbacks, accurate X link sharing, and accessible text |
...
| PROF-52 | Share dialog = current product composition: title row + close, card preview, **Save image** (primary) / **Copy image** / **Share** (system share sheet; unsupported browsers fall back to the X Web Intent per this contract). Vessel = C8 `ProfileShareDialog` (overlay surface, `border/overlay`, radius 10, overlay elevation, layer-header 24/16). States: Default · Preparing (skeleton preview, disabled actions, `profile.preparingImage`) · Copied (Success status) · ImageError (Danger status, actions kept) · CardError (Danger status + Retry). Compact 334 / Wide 768 — the 768 is borrowed from the reading container and is not yet an approved overlay width | `Approved — 2026-09-06` (Z1 ㉓)       |
...
### Decision Log addendum — 2026-09-06 (Codex V05 · Z1 ㉖)

| ID      | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PROF-53 | **Stacked surfaces removed.** The identity block and the four body sections (trend, best plays, record overview, recent plays) no longer sit on `surface/surface` cards with their own inset; they sit on the canvas with `section-title` and 48 px stack spacing as the boundary. Only the competitive summary keeps its card, so it is the one emphasised face. Rationale: in Light the card face reads 1.06:1 against the canvas (the value already judged invisible for `DISC-44`), so the card hierarchy existed in Dark only; opening the cards makes both modes the same structure, and it removes the card-inside-card repetition of the play rows. Body content reflows to the full column (stacked judgement bar re-derived by largest remainder; 1280 grid rows re-derived from content). Not a file-wide rule — faces kept for contrast (the P1 radar panel, `Q1-A`) stay. |
| PROF-54 | **Mode selector outside the card.** The `Basic / Recital` SegmentedControl applies to the whole Profile, so it sits above the competitive-summary card (gap 16) at the full content width (358 compact), not inside the card. User decision from the 1.0 layout.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| PROF-55 | **Public metadata as chips.** NOSTALGIA ID, Discord, preferred arcade and last-played each become a small container — `surface/surface`, `radius/container`, inset 12, content hugged, wrapped with gap 8 — the Home tile language (`HOME-22`). Hugging width was chosen over equal columns because at 390 the widest chip (label + date, 179) exceeds a half column (175) and at 320 two columns fail outright; hugging fits every width and locale without a derivation. Labels stay text (no icon-only reduction, per the Compact clause). Visibility caveat: like the Home tiles, the chip face is near-invisible in Light (1.06:1) and reads as a container in Dark.                                                                                                                                                                                                              |

### Decision Log addendum — 2026-09-06 (Z1 ㉗ · exam grade badge)

| ID      | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PROF-56 | **Exam grade badge = tier plate** (C3 `ExamBadge`). A 24 px plate (inset 4, gap 8) on `surface/plate` with a 6 px left band in the grade's tier colour, the mode initial `B` / `R` (`nav-fit-current/latin`, 600) and the grade `N급` (`metadata`), both `content/default`. Tier bands: 10–8급 `exam/tier-low`, 7–5급 `tier-mid`, 4–3급 `tier-high`, 2급 `tier-top`, 1급 `tier-peak`. Chosen from ten drawn options (colour pill, hexagon, belt, medal, ribbon, shield, seal, piano key, progress ring) against 段位 plate conventions (taiko 段位道場, IIDX 段位, SDVX, CHUNITHM, maimai) and rank badges outside rhythm games (osu!, Steam, Overwatch, Valorant, Rocket League, Duolingo, Codeforces, chess.com). The mode is carried by the initial, not by colour, so the tier ramp stays one axis. Deviation from the drawing: the initial is neutral rather than tier-coloured, because no tier colour in the file reaches 4.5:1 as 12 px text on a visible plate face; the band is non-text and clears 3:1 in both modes. |

### Decision Log addendum — 2026-09-06 (identity block tidy-up · user direction from the 1.0 profile)

| ID      | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| PROF-57 | **Owner actions top-right, nickname-sized.** Share and Settings sit at the right end of the avatar/name row, top-aligned — the 1.0 placement — as **32×32** icon controls (icon 20), the height of the name line. The user chose 32 over the 40/44 icon-target rule because the 40 pair at 320 pushed the exam badges out of the row; recorded as a bounded exception for these two owner controls. Row gap 16. Visitors have no row change (the actions are absent).                                                                                                                                              |
| PROF-58 | **One meta line under the avatar row**: `마지막 플레이 {date} · 동기화 {relative}` in `body-secondary` `content/subdued`. The last-played chip and the separate sync row are gone. The sync half is owner-only; the last-played half follows the play-activity visibility control; a part that does not apply is dropped with its separator. Sync copy loses the `마지막` prefix (`동기화 3일 전` / `同期 3日前` / `Synced 3 days ago`) — `PROF-40` semantics unchanged (NosLog's own observation only); the other sync sentences (never synced, syncing, partial, failed) keep their wording after the separator. |
| PROF-59 | **NOSTALGIA ID chip spans the full row**; Discord and preferred-arcade chips follow on the next line as **two equal FILL columns** (gap 8). Where the widest chip does not fit its column (320: 153 > 140) the pair stacks vertically, full width. Same rule on every width.                                                                                                                                                                                                                                                                                                                                       |
